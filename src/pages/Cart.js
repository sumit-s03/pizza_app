...
const Cart = () => {
    ...
    const [priceFetched, togglePriceFetched] = useState(false); // for network request, every time quantity incremented so fetch request called, to prevent it we are using flag or check here.
    useEffect(() => {
        if (!cart.items) {
            return;
    ...
    return (
        !products.length 
        ? <img className="mx-auto w-1/2 mt-12" src="/images/empty-cart.png" alt="cartt"/>
        :
